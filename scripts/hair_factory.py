...
원문 전체에서 얼굴형, 성별, 삼정 비율, 헤어스타일, 추천/비추천 표현을 찾습니다.

조건과 스타일이 명확히 연결된 문장 또는 문단만 후보로 삼습니다.

후보별로 성별, 얼굴형, 삼정 비율을 표준값으로 정규화합니다.

원문 스타일명을 표준 헤어스타일 목록의 style_group 코드와 style_name으로 매핑합니다.

추천 스타일과 비추천 스타일을 분리합니다.

원문 표현을 복사하지 않고, 추천/비추천 이유를 새롭게 재작성합니다.

최종 JSON 스키마에 맞는 항목만 출력합니다.

출력 직전 모든 필드값이 허용된 값인지 검증합니다.
...
한 원문에서 여러 얼굴형에 대한 추천이 명확히 등장하면 JSON 객체를 여러 개 생성하세요.

같은 얼굴형과 같은 성별, 같은 삼정 비율 조건에 여러 추천 스타일이 있으면 recommended_styles 배열에 여러 스타일 객체를 넣으세요.

같은 조건에 추천 스타일과 비추천 스타일이 모두 있으면 하나의 JSON 객체 안에 recommended_styles와 worst_styles를 함께 넣으세요.

얼굴형 조건이 다르면 반드시 별도 JSON 객체로 분리하세요.

성별이 다르면 반드시 별도 JSON 객체로 분리하세요.

삼정 비율 조건이 다르면 반드시 별도 JSON 객체로 분리하세요.

같은 조건의 객체가 여러 개 생길 경우 가능하면 하나로 병합하세요.
...
"raw_style_hint": "원문에서 확인된 짧은 스타일 단서",
...
JSON 외의 문장이 포함되지 않았는가?

category는 반드시 "hair"인가?

gender는 "여성" 또는 "남성"인가?

face_shape는 허용된 5개 값 중 하나인가?

face_proportion은 허용된 4개 값 중 하나인가?

recommended_styles와 worst_styles는 배열인가?

모든 style_group은 f-01~f-35 또는 m-01~m-21 중 하나인가?

모든 style_name은 표준 목록의 스타일명과 정확히 일치하는가?

여성 gender에 남성 스타일 코드가 들어가지 않았는가?

남성 gender에 여성 스타일 코드가 들어가지 않았는가?

raw_style_hint에 연예인명, 인플루언서명, 브랜드명, 채널명, 문장형 원문 표현이 들어가지 않았는가?

expert_reasoning_positive와 expert_reasoning_negative에 원문 문장, 인용문, URL, 소스 번호, 마크다운 기호가 들어가지 않았는가?

원문에 근거가 없는 추천 조합을 새로 만들지 않았는가?

추천 이유가 없는 객체를 만들지 않았는가?

mapping_confidence가 0.60 미만인 스타일 객체를 만들지 않았는가?
